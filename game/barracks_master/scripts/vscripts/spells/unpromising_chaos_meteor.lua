...
function invoker_chaos_meteor_datadriven_on_spell_start(keys)
    ...
    Timers:CreateTimer({
        ...
        callback = function()
            ...
            chaos_meteor_dummy_unit:AddAbility("unpromising_chaos_meteor")
            local chaos_meteor_unit_ability = chaos_meteor_dummy_unit:FindAbilityByName("unpromising_chaos_meteor")
...
function modifier_invoker_chaos_meteor_datadriven_main_damage_on_interval_think(keys)
    local nearby_enemy_units = FindUnitsInRadius(keys.caster:GetTeam(), keys.caster:GetAbsOrigin(), nil, keys.AreaOfEffect, DOTA_UNIT_TARGET_TEAM_ENEMY,
        DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
    ...
    if keys.caster.invoker_chaos_meteor_parent_caster ~= nil then
        for i, individual_unit in ipairs(nearby_enemy_units) do
            ...
            ApplyDamage({victim = individual_unit, attacker = keys.caster.invoker_chaos_meteor_parent_caster, damage = keys.caster.invoker_chaos_meteor_main_damage, damage_type = DAMAGE_TYPE_PHYSICAL,})
...
function modifier_invoker_chaos_meteor_datadriven_burn_damage_on_interval_think(keys)
    if keys.caster.invoker_chaos_meteor_parent_caster ~= nil and keys.caster.invoker_chaos_meteor_burn_dps ~= nil then
        ApplyDamage({victim = keys.target, attacker = keys.caster.invoker_chaos_meteor_parent_caster, damage = keys.caster.invoker_chaos_meteor_burn_dps * keys.BurnDPSInterval, damage_type = DAMAGE_TYPE_PHYSICAL,})
    end
end